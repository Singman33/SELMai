...
api.interceptors.response.use(
  (response) => response,
  (error) => {
    // Ne pas déconnecter automatiquement sur certaines erreurs 401
    // Par exemple, si l'utilisateur entre un mauvais mot de passe actuel lors du changement
    const isPasswordChangeError = error.config?.url?.includes('/users/profile/password');

    if (error.response?.status === 401 && !isPasswordChangeError) {
...
export const userAPI = {
  ...
  updateProfile: async (email: string, firstName: string, lastName: string) => {
